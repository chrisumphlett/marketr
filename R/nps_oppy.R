...
#'
#' Calculate how much NPS would increase if each distinct value of an attribute
#' had a perfect NPS score. Optionally choose a grouping column to do it by
...
#'
#' @return Data frame with baseline NPS and how much NPS would increase by if
#' a given attribute had perfect NPS scores.
#'
#' @import dplyr
#' @importFrom magrittr "%>%"
#' @import tidyr
  ...
  inner_fun <- function(.x) {
    ...
    nps_df <- {{survey_data}} %>%
      dplyr::mutate(nps_orig = .data$nps_question,
             nps_perfect = 10.0,
             nps_question = dplyr::if_else({{opp_var}} == .x,
                                           as.numeric(.data$nps_perfect),
                                           as.numeric(.data$nps_orig)))
