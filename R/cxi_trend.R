...
#'
#' Simplifies the calculation of Customer Experience Index (CXi) trends over
#' time from raw survey data within the tidyverse framework.
#'
#' Customer Experience Index is a metric created by Forrester to help
#' companies systematically measure customer experience in a way that their
#' research has found is connected to improving customer loyalty. More
#' information can be found at https://go.forrester.com/analytics/cx-index/
#'
#' The calculation across an entire sample of surveys is simple. A customer
#' experience manager may want to calculate CXi across many different
#' dimensions and filtering in different ways; the functions in this package
#' utilize the tidy framework to streamline calculating CXi along as many
#' dimensions as desired.
#'
#' The trend version of the function allows you to specify one column as a date
#' over which to trend the data. This allows quick filtering to eliminate
#' groupings that fail to meet user-specified thresholds for average or minimum
#' survey counts per time period.
#'
...
#' three CXi question responses having column names of needs, ease and emotion
#' @param trend_var Column that represents an element of time, eg week number,
#' date, month & year
#' @param ... optional columns by which to group the CXi calculation. There is
#' no limit to the number of grouping variables chosen. Too many will likely
#' result in CXi calculations that are too fragmented / based on very small
#' survey counts.
...
#' be included
#'
#' @return Data frame with CXi and survey count for each combination of the
#' grouping variables over the time variable.
#'
#' @import dplyr
#' @importFrom magrittr "%>%"
#' @import tidyr
...
#' cx_date <- rep(seq.Date(from = as.Date("2019-01-01"),
...
cxi_trend <- function(survey_data, trend_var, ..., cx_high = 4, cx_low = 2,
                      min_surveys = 1, avg_surveys = 0) {
  cx_high2 <- cx_high
  cx_low2 <- cx_low

  . <- NA # prevent variable binding note for the dot

  survey_transpose <- survey_transpose_cxi(survey_data, cx_high = cx_high2,
                                           cx_low = cx_low2, ...)

  cols <- c(HIGH = NA_real_, MID = NA_real_, LOW = NA_real_)

  cxi <- survey_transpose %>%
    dplyr::group_by(..., {{trend_var}}, .data$question,
                    .data$response_class) %>%
  ...
  cxi2 <- cxi %>%
    ...
    dplyr::summarise(cxi = mean(.data$question_score),
                     survey_count = sum(.data$HIGH + .data$LOW + .data$MID)
                     / 3) %>%
    dplyr::ungroup()
  ...
  test <- tryCatch({
    ...
    dplyr::group_by(...) %>%
      ...
      dplyr::select(..., {{trend_var}}, cxi, .data$survey_count,
